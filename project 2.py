...
def get_high_lows_lookback(high, low, lookback_days):
    ...
    df_rolling_high = high*0

    df_rolling_low = low*0
    
    
    for stock in high:
        
        stock_rolling_high = pd.DataFrame(high[stock].rolling(lookback_days).max())

        df_rolling_high[stock] = stock_rolling_high.dropna()

        lookback_high = df_rolling_high[:-1].fillna(method= 'bfill')
        
        
        stock_rolling_low = pd.DataFrame(low[stock].rolling(lookback_days).min())

        df_rolling_low[stock] = stock_rolling_low.dropna()

        lookback_low = df_rolling_low[:-1].fillna(method= 'bfill')
...
def get_long_short(close, lookback_high, lookback_low):
    ...
    for x in range(close.shape[0]):
        for y in range(close.shape[1]):
            if close.iloc[x,y] > lookback_high.iloc[x,y]:
                long_short.iloc[x,y] = 1
            if close.iloc[x,y] < lookback_low.iloc[x,y]:
                long_short.iloc[x,y] = -1    
...
def filter_signals(signal, lookahead_days):
    ...
    """
    signew = pd.DataFrame(0,index = signal.index, columns = signal.columns)
    for key,value in signal.iteritems():
        siglong = pd.Series(data = 0,index = value.index)
        siglong[value == 1] = 1
        siglong = clear_signals(siglong, lookahead_days)
        
        sigshort = pd.Series(data = 0,index = value.index)
        sigshort[value == -1] = 1
        sigshort = clear_signals(sigshort, lookahead_days)
        
        signew[:][key] = siglong - sigshort
...
def get_signal_return(signal, lookahead_returns):
    ...
    signal_returns=signal*lookahead_returns
...
# Returns dont seem to follow a normal distribution, when days increase the distribution seem to follow less a normal distribution, and outliers increase too
...
def calculate_kstest(long_short_signal_returns):
    ...
    sd = long_short_signal_returns['signal_return'].std(ddof = 0)
    ...
    for x,group in grouped:
        k,p= kstest(group['signal_return'],'norm',args = (mean,sd))
